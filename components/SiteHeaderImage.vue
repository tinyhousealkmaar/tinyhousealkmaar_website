<template>
  <div class="p-4">
    <header class="flex justify-between flex-row xl:grid grid-cols-12 gap-2">
      <div class="col-start-1 col-span-3">
        <nuxt-link to="/"
          ><img
            ...
            class="logo w-32 h-auto brightness-0 invert hover:invert-0"
        /></nuxt-link>
      </div>
      ...
      <div
        class="menu xl:hidden bg-white w-screen h-[50vh] absolute top-0 left-0 z-10 p-8 border-b-[#DBEEE1] border-b-4"
      ...
      <div class="hidden xl:inline-block col-start-4 col-span-6 -ml-2">
        <ul class="sm:flex items-center h-full">
          <template v-for="(item, index) in menu">
            <li :key="index">
              <nuxt-link
                class="p-2 px-3 hover:bg-slate-900/50 rounded-md text-lg text-white font-semibold"
                :to="item.page"
                >{{ item.text }}</nuxt-link
              >
            </li>
